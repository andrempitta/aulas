...
class Pessoa(BaseModel):
    ...
    nome: constr(min_length=3, max_length=10) # Constrained Str
    sobrenome: Optional[constr(min_length=3, max_length=20)]
    idade: int = Field(
        gt=0,
        lt=150,
...
def buscar_pessoas():
    """ Retorna todas as Pessoa(s) da base de dados"""
    ...
    return jsonify(
        Pessoas(
            ...
            count=len(todas_as_pessoas)).dict()
        )
...
def altera_pessoa(id):
    """ Altera a Pessoa no bando de dados."""
    Pessoa = Query()
    body = request.context.body.dict()
